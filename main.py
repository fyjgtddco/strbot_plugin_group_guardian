...
from astrbot.api.message_components import Plain, At  # 去掉了 Comp
...
class GroupGuardianPlugin(Star):
    ...
    async def cmd_mute(self, event: AstrMessageEvent, target: str = "", duration: int = 60):
        ...
        if self._is_protected(t):
            ...
            return
        chain = MessageChain([Plain(f"[CQ:ban,qq={t},duration={duration}]")])
        yield chain
    ...
    async def cmd_wl(self, event: AstrMessageEvent):
        ...
        if act == "add":
            if target in cur:
                yield event.plain_result(f"⚠️ 已在白名单")
            else:
                cur.append(target)
