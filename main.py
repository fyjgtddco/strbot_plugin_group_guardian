...
=====================================
所有指令在 on_llm_request 中处理，匹配后阻止 LLM。

指令：
  /群管帮助 | /群管状态
  /禁言 @xxx <秒数> | /解禁 @xxx | /踢人 @xxx | /全体禁言
  /群管白名单 add @xxx | del @xxx | list
"""
...
class GroupGuardianPlugin(Star):
    def __init__(self, context: Context, config: AstrBotConfig):
        ...
        logger.info(f"[GroupGuardian] 管理员: {self._admins()}")
    ...
    def _is_admin(self, qq: str) -> bool:
        admins = self._admins()
        result = qq in admins
        logger.info(f"[GroupGuardian] _is_admin({qq}) -> {result}, admins={admins}")
        return result
    ...
    async def _send_cq(self, event: AstrMessageEvent, cq_text: str):
        """发送 CQ 码文本（禁言/踢人/全体禁言）"""
        await self._reply(event, cq_text)
    ...
    async def on_llm_request(self, event: AstrMessageEvent, *args, **kwargs):
        ...
        logger.info(f"[GroupGuardian] 消息: {raw[:80]}, sender={sender}")
        ...
        if clean.startswith("/群管状态"):
            ...
            await self._reply(event,
                ...
                f"白名单：{len(w)}人\n"
                f"发送者：{sender} | 是管理员：{self._is_admin(sender)}")
            return

        if clean.startswith("/禁言"):
            event.stop_event()
            await self._handle_mute(event, raw, sender)
            return

        if clean.startswith("/解禁"):
            event.stop_event()
            await self._handle_unmute(event, raw, sender)
            return

        if clean.startswith("/踢人"):
            event.stop_event()
            await self._handle_kick(event, raw, sender)
            return

        if clean.startswith("/全体禁言"):
            event.stop_event()
            await self._handle_whole(event, sender)
            return

        if clean.startswith("/群管白名单"):
            event.stop_event()
            await self._handle_wl(event, raw, sender)
            return
    ...
    # ==================== 指令处理 ====================
    async def _handle_mute(self, event, raw, sender):
        if not self._is_admin(sender):
            await self._reply(event, f"❌ 权限不足（sender={sender}）")
            return
        ...
        if self._is_protected(t):
            ...
            return
        dur = 60
        m = re.search(r'(\d+)\s*[sS秒]?\s*$', raw)
        if m:
            dur = int(m.group(1))
        await self._send_cq(event, f"[CQ:ban,qq={t},duration={dur}]")
        await self._reply(event, f"✅ 已禁言 {t}，{dur}s")

    async def _handle_unmute(self, event, raw, sender):
        if not self._is_admin(sender):
        ...
        if not t:
            ...
            return
        await self._send_cq(event, f"[CQ:ban,qq={t},duration=0]")
        await self._reply(event, f"✅ 已解禁 {t}")

    async def _handle_kick(self, event, raw, sender):
        if not self._is_admin(sender):
            await self._reply(event, "❌ 权限不足")
            return
        t = self._extract_at(raw)
        ...
        if self._is_protected(t):
            await self._reply(event, "❌ 该成员受保护")
            return
        await self._send_cq(event, f"[CQ:kick,qq={t}]")
        await self._reply(event, f"✅ 已踢出 {t}")

    async def _handle_whole(self, event, sender):
        if not self._is_admin(sender):
            await self._reply(event, "❌ 权限不足")
            return
        await self._send_cq(event, "[CQ:whole_ban]")
        await self._reply(event, "✅ 已切换全体禁言")

    async def _handle_wl(self, event, raw, sender):
        if not self._is_admin(sender):
            await self._reply(event, "❌ 权限不足")
            return
        clean = self._strip_cq_at(raw)
    ...
    async def _flood_check(self, event, gid):
        ...
        if len(t) > mx:
            ...
            await self._send_cq(event, f"[CQ:ban,qq={qq},duration={dur}]")
        ...
        if self._cfg("ai_auto_mute_after_warn"):
            for u in users:
                ...
                if 10 < now - last < 300:
                    ...
                    await self._send_cq(event, f"[CQ:ban,qq={u},duration={d}]")
