...
class GroupGuardianPlugin(Star):
        ...
        try:
            for comp in event.message_obj.message:
                if isinstance(comp, At):
                    qq = str(comp.qq)
                    if qq and qq != "0":  # 排除 @全体成员
                        return qq
        except Exception as e:
            logger.error(f"[GroupGuardian] _extract_at 失败: {e}")
        return None
    ...
    def _clean_cmd(self, msg: str) -> str:
        """去掉消息中 @ 开头的部分，提取纯指令"""
    ...
    async def on_llm_request(self, event: AstrMessageEvent, *args, **kwargs):
        ...
        if clean.startswith("/禁言"):
            event.stop_event()
            await self._handle_mute(event, clean, sender)
            return

        if clean.startswith("/解禁"):
            event.stop_event()
            await self._handle_unmute(event, sender)
            return

        if clean.startswith("/踢人"):
            event.stop_event()
            await self._handle_kick(event, sender)
            return
        ...
        if clean.startswith("/群管白名单"):
            event.stop_event()
            await self._handle_wl(event, clean, sender)
            return
    ...
    async def _handle_unmute(self, event, sender):
        if not self._is_admin(sender):
    ...
    async def _handle_kick(self, event, sender):
        if not self._is_admin(sender):
            await self._reply(event, "❌ 权限不足")
            return
        t = self._extract_at(event)
    ...
    async def _handle_wl(self, event, clean, sender):
        if not self._is_admin(sender):
            await self._reply(event, "❌ 权限不足")
            return
        parts = clean.split()
        ...
        target = self._extract_at(event) or (parts[2] if len(parts) > 2 else "")
